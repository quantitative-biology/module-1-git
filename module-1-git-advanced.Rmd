...
Realise that you still edit and save your files in the usual way on your
  ...
  of your files on your computer (as you would if you weren't using Git).

### I've made some changes but don't want to keep them -- stash
...
(such that they can maybe be retrieved later -- you don't have to fully delete them):
```
git stash
```
...
sometimes you may later wish you had kept them somehwere. You can have multiples
stashes, seen by doing:
```
git stash list
```
...
TODO: see my README for details, and test them.
...
* Look at the commit tab on GitHub for your test repo and click on the clipboard
      icon to copy the HASH to the clipboard .
* In Git shell: `git checkout HASH`
...
Errr... but now I want my work  back please...
...
That's it! Check that your files are back. So you can `r colorize("revert to any previous commit in your
    repository").
...
So you can transit the commit history and select any previous version of your
  repository. So your workflow is less cluttered and more tractable than having
  to save multiple versions of the same files with dates in the filename, such as
```{r, versions}
knitr::include_graphics("../module-1-git/images/EAversions.png")
```
...
first two digits of a HASH. The rest of digits of the HASH are the filenames in the
subdirectory.

You can think of the hashes as representing `commits` (apparently they can also
be `blobs` and `trees` but I've not looked into understanding that). I
think of these files as containing the `r colorize("differences")` between each commit.
...
Exceptions: often you may have an image (or other) file that you need for a
      write up, but it isn't going to keep changing. So that's fine to commit.
...
* A collaborator was running some R code (and correctly committed the `.R` files
  so that others could run it), but also committed the results, which included
...
* But, these latter files got updated every time the code was run. So changing
  ...
  saved (since Git cannot just save the difference from the last commit). Even
  if, say, one point changes on a figure in a graph in a `.pdf` file.

* This ended up with `.git/objects/pack` (whatever that might be!) being 2.8Gb.
* I needed space quickly so just deleted four files in `.git/objects/pack`, which freed up 1.6Gb.
...
'fatal' errors with scary messages like `bad object HEAD` and the great 'You are
on a branch yet to be born':
```{r, notgood}
knitr::include_graphics("../module-1-git/images/notGood.png")
```
...
questions are posted and answered on the `stackoverflow` website).
A bit more understanding of terminology will help you.

Git does not keep versions of code, it keeps _commits_. The commits
are kept track of using a hash code, using Secure Hash Algorithm 1
(SHA-1) which generates a hash key 40 digits long in hexadecimal (base 16). These are
