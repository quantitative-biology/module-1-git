...
exercises for you (you need a partner for the second one).
}
...
\frame{\frametitle{Creating and committing a new file}
  \bi
    \item Create a new file, \emph{newFile.txt}, in your \lst{github/test} directory.
    \pause
  \item Check the status of your (\lst{test}) repository by typing
...
\frame{\frametitle{Exercise: create, edit, and commit \emph{simpleText.txt}}
  \bn
    \item Create a text file \lst{simpleText.txt}
      in your local \lst{test} repository.
    \pause
...
\frame{\frametitle{The \emph{.gitignore} file}
  ...
  created, then create (and add and commit) a \emph{.gitignore} file with \emph{.DS\_Store} as a line.
}
