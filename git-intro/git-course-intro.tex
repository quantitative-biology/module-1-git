...
\frame{\frametitle{Basics}
  \bigskip
  I will go through the next 7-8(TODO) slides, demonstrating the basic commands.\\
  ~\\
  Some are only needed once per repository to get set up.\\
  ~\\
  Then you should follow the slides yourself, to become familiar with the basic ideas.\\
  \bigskip
}
...
\frame{\frametitle{Copy and commit \emph{.gitignore}}
  \bi
    \item Copy the \emph{.gitignore} file from the \lst{git-course} directory and
      paste it into your new \lst{github/test} directory. (We are using
      \emph{.gitignore} as an example file here).
    \pause
 ...
 \item You should see that the file is listed as a `new file` under `Changes to
   be committed`.
    ...
    \pause
    \item Check (refresh) the \gh\ webpage and see your commit and that the file
      has been uploaded.
  \ei
}
\endgroup
...
\frame{\frametitle{Keyboard shortcuts}
Now, \lst{git commit -a -m "Message."} is a bit much to type, so we have a
shortcut:
...
realise it is a shortcut if searching online for help.

Similarly:

\lst{git s} -- shortcut for \lst{git status}

\lst{git p} -- shortcut for \lst{git push}

From now on we will use the shortcuts.

TODO: say how to simplify to \lst{g s} etc.
}
...
\frame{\frametitle{Edit \emph{Readme.md}}
  ...
  repository:\\
  \lst{git status} (or just \lst{git s})\\
  ...
  \lst{git push} (or just \lst{git p}\\
  ~\\
...
\frame{\frametitle{Copy, edit and commit \emph{simpleText.txt}}
  \bn
    ...
    \pause
    \item \lst{git com "Message"} frequently and \lst{git push}
...
\frame{\frametitle{Aliases}
  ...
  \lst{git com "MESSAGE"}   & \lst{git com "MESSAGE"}\\
