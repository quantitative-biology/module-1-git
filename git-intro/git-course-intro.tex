...
\frame{\frametitle{Git Workflow}
 ...
 Concept: there is a project where people contribute to a main repository that is
      ...
      \bi
        ...
        \item All collaborators require push their commits to the main
...
\frame{\frametitle{Demonstration of collaborating}
  \bi
    \item Andy to clone Kim's \lst{test} repo.
    \item Kim gives Andy `push access' to her \lst{test} repo.
    % \item Both add each other as remotes.
    \item Both do some edits (Andy create new files, Kim edit existing ones).
    ...
    \bi
      ...
      \item \lst{git rebase}  -- this updates your local
        repository (the files on your computer) with the changes you
...
We will show examples of:
  \bi
  \item \lst{git p} not being allowed (read the message the shell gives you!)
  because there are changes on \gh\ that you have not yet merged into your local
  repository. You need to deal with these first by \lst{git f} and then \lst{git rebase}.
  \item {\red Conflict!} This happens when two people have edited {\red the same line(s)
      of the same file}.
...
\frame{\frametitle{Fixing a conflict}
  TODO: Set up some text to use and say where to have conflicts

We will demonstrate a conflict. Kim will edit a line show examples of:

(mention \lst{git merge} somewhere)
