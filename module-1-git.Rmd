...
`r colorize("GitHub")` refers to the website [github.com]("http::github.com")
...
and collaborate with anyone. `r colorize("repository")` is generally used
interchangeably to mean both the collection of code and the
...
Each GitHub repository can be viewed on GitHub. We will take a tour of a GitHub
repository in the following video, that shows:
...
TODO - video: sizeSpectra to show files, README, can save individual files (copy
raw contents button then save manually - need for .gitconfig) can
download .zip of whole thing. Then hake-assessment to show multiple
...
* We have called this a [TTT approach](https://waves-vagues.dfo-mpo.gc.ca/Library/40750152.pdf):
...
knitr::include_graphics("./images/hake-picture.png",
                        error = FALSE)
```
...
knitr::include_graphics("./images/interim.png",error = FALSE)
```
...
knitr::include_graphics("./images/EAversions.png",error = FALSE)
```
...
knitr::include_graphics("./images/proposalEmail.png",error = FALSE)
```
...
knitr::include_graphics("./images/dogsBreakfast.png",error = FALSE)
```
...
knitr::include_graphics("./images/MEEcodeQuest.png",error = FALSE)
```
...
knitr::include_graphics("./images/MEEcodeAnswer.png",error = FALSE)
```
...
```{r, echo = FALSE, fig.alt = "Example of GitHub showing who wrote each last line of a file.", width="100%", fig.align = "center"}
knitr::include_graphics("./images/blame.png",error = FALSE)
```
...
knitr::include_graphics("../module-1-git/images/scooby-blame.png",error = FALSE)
```
...
knitr::include_graphics("./images/hakeIssues21.png",error = FALSE)
```
...
I use [magit](https://magit.vc/) which works in the text editor emacs (which for years I have used
...
the 'power' part means it's more powerful than a basic version), Git, and then
...
anyone can see the code), but when you scroll down you see instructions (as per
the example in the first video).
...
* Windows
  ...
  1. Create a local GitHub directory, such as `C:\github`.  It is fine to put it
     in a different path, but make sure there are no spaces or special
     characters anywhere in the full path. This is where you want to be saving
     your work that you are tracking with Git. You don't have to save Git
     repostitories in this folder, but, especially at first, it can be helpful
     so you remember that you are tracking a folder with Git.

  2. TODO: Andy has to reinstall anyway and will write something here. Think it's just following instructions.

* MAC
  1. Create the directory ~/github for putting everything you're going to track
     with Git (see 1. above).


### Install a difftool (optional)

A difftool will be used to examine differences between different versions of
...
The configuration for directing Git to use Diffmerge will be done below. TODO: check

### Save our template _.gitconfig_ file

* Git uses a configuration file (`.gitconfig`) for your account info, name to use when
...
* For Windows, you will save is as
...
* The template file is on GitHub
  [here](https://raw.githubusercontent.com/quantitative-biology/module-1-git/main/misc/.gitconfig). Do
...
* Just a reminder for your understanding: that file on GitHub is
  ...
  code for the file -- this works for any file on GitHub, as mentioned in the
  opening video.

### Edit the _.gitconfig_ file

* Use your text editor to open the new file. Given it has a non-standard
...
* If you did not install [difftool] or [diffmerge] then delete those lines in your _.gitconfig_ file.
* TODO: check if need to keep the original [credential] part.
...
set up. To enable this shortcut, create a file (on Windows at least) called `g.bat`
