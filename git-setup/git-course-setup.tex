...
\frame{\frametitle{While testing...}
  ...
  Luwen, Matt etc. -- please keep notes of any steps that aren't clear. I can't
troubleshoot the installation, so any feedback is welcome.
}
...
\frame{\frametitle{TODO: For troubleshooting}

  Please keep track of any installation difficulties and let me know, so we can update
  the slides to make the instructions easier for people.

~\\
There might be some one-time authentication to your GitHub account (the first
time you get set up), that I think
may be prompted by instructions. Follow those if necessary, and please let Andy
know so we can make a note here. (Once we're all using Git you'll know how to make the
change yourself, rather than tell me and I make it!)
...
\frame{\frametitle{Configure the Git application}
  ...
  \bi
    ...
    \item Create a \lst{github} directory, such as
        ...
        \item TODO: think these Windows steps are simpler nowadays. Basically
...
\frame{\frametitle{Markdown Pad [Nice to have - not essential if you
    have trouble with it]}
...
\frame{\frametitle{To keep notes}
  ...
  end up making some amazing changes to our files that you want to share).
}

\frame{\frametitle{You're ready...}
  That's it! You should have everything set up to create new repositories,
  commit changes, push your commits, create branches, collaborate, and view
  differences.

  ~\\

  During the course you'll understand what all that means.\\
  % \bigskip
